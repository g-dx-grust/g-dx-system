...
10. **ダッシュボードのメトリクスカードはラベル→数値の順に配置する（日本式）**

    * 欧米UIでは「数値を上、ラベルを下」に置く設計が一般的だが、日本では「ラベル（題名・内容）を先に示してから数値を見せる」読み方が自然なため、**カード内の縦順はラベルを上、数値を下**にする。

    ```
    ✅ 日本式（本プロジェクト採用）   ❌ 欧米式（採用しない）
    ┌──────────┐                 ┌──────────┐
    │  進行中   │                 │    0     │
    │    0     │                 │  進行中   │
    └──────────┘                 └──────────┘
    ...
    * この順序はすべてのダッシュボードカード・KPIウィジェット・サマリーバッジに統一して適用する。
    * 例外（アイコン、バッジカラー等の補助要素）はラベルと数値の間ではなく、ラベルの横に配置する。
